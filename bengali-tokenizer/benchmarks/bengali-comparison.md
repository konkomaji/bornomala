...
BMBT (Bornomala's Bengali Tokenizer, `bntok/bmbt.py`) is grammar (the akshara parser above) plus a featural decomposition (`featurize()`) plus a statistical BPE layer over akshara atoms - the same architecture as v1, with the atomic unit swapped from grapheme cluster to akshara. Morphology is now built as well, opt-in, and aligns token boundaries to Bengali's suffix structure: see `docs/bmbt-morphology.md`. Full design: `docs/bmbt-architecture.md`.
