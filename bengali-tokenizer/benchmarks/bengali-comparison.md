...
## v2 design step 4: the akshara finite-state parser, measured (not a like-for-like row)

The v2 design's own build sequence (`docs/design/reading-bengali-on-its-own-terms.md`) calls for benchmarking the akshara parser (`bntok/akshara.py`) before building anything on top of it. Measured via `scripts/compare.py`'s `measure_akshara()`, on the exact same held-out sets as the tokenizer results above:
...
**This is not a like-for-like comparison and is reported separately on purpose.** The akshara parser has no vocabulary and no merges (v2 design step 5's statistical layer is what BMBT is): its "fertility" is the number of un-merged akshara/other chunks per word, the pre-compression granularity, not a trained tokenizer's post-BPE-merge token count. Needing roughly 3-4x more units per word than the trained BPE tokenizers is expected, not a regression - exactly the number step 4 asked to be measured and reported honestly before proceeding to step 5.
...
## v2 design step 5: BMBT, measured (a genuine like-for-like row)
